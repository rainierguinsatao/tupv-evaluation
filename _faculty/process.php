...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($_POST['question'] as $qid => $score) {
        ...
        if ($selectOption == 'Supervisor' || $selectOption == 'Peer to Peer' || $selectOption == 'Self') {
            $sql = "INSERT INTO rate_score_tbl (type, name, course, term, sy, qid, tits, faculty, score) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        } else {
        ...
        if ($selectOption == 'Self') {
            ...
            $stmt->bind_param("ssssssssi", $selectOption, $fn, $course, $term, $schoolyear, $qid, $tits, $fn, $score);
        } else {
            // Use $faculty_to_eval as the value for the 'faculty' column
            $stmt->bind_param("ssssssssi", $selectOption, $fn, $course, $term, $schoolyear, $qid, $tits, $faculty_to_eval, $score);
        }
